...
These are proposed bridge designs, **not claims of tested compatibility**.
Only the Python adapter has installed-package local circuit acceptance here;
the Go starter is build/unit-tested. No third-party platform test was run in this
pass, and no provider account or hosted-agent subscription was configured.
...
| Hermes Agent | [MCP integration](https://hermes-agent.nousresearch.com/docs/user-guide/features/mcp) | Add the adapter to the Python runtime's incoming-event path. Verify a ring wakes the decision loop and that call text bypasses conversation history and memory creation. The ordinary MCP server entry alone is not a receiving bridge. |
